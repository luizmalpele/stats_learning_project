...
college_data %>% 
  ...
  select(name, make_world_better_percent) %>% 
  ggplot() +
  geom_boxplot(mapping = aes(y = make_world_better_percent)) -> arts_box

college_data %>% 
  filter(!is.na(make_world_better_percent), grepl(" health ", name, ignore.case = T) == T | 
           grepl(" nursing ", name, ignore.case = T) == T) %>% 
  select(name, make_world_better_percent)  %>% 
   ggplot() +
   geom_boxplot(mapping = aes(y = make_world_better_percent)) -> health_box
